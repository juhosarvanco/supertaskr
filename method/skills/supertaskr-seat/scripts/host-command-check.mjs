...
function selftest() {
  ...
  writeFileSync(
    ...
    [
      ...
      "- THE THIRD: `node tools/x/third.mjs --once` from tools/x/ and nowhere else.",
      "",
    ].join("\n"),
  );
  writeFileSync(
  ...
  const good = [
    ...
    "### the third",
    "",
    "AUTHORITY: `docs/CONVENTIONS.md`, THE THIRD bullet.",
    "",
    "    HOST> node tools/x/third.mjs --once",
    "    CWD> from tools/x/",
    "",
    "### the board",
    "",
  ...
  // DECLARED LIMITS — measured in this output rather than claimed in prose
  // (T-241's verdict, correction 2): the CWD> phrase is resolved against the
  // whole corpus, UNBOUND to its command, so a RIGHT command paired with a
  // WRONG directory the corpus states for ANOTHER command is NOT caught. The
  // check is a transcription test, not a pairing test; the pairing is the
  // reader's, against the bullet. A limit that is one day caught is a stale
  // declaration, and this selftest FAILS on it so the prose moves with the
  // behaviour.
  const declaredLimits = [
    [
      "RIGHT command, WRONG cwd that the corpus states for ANOTHER command (the runner repointed to tools/x/)",
      good.replace(
        "    HOST> node tools/x/run.mjs --all\n    CWD> from the repo root",
        "    HOST> node tools/x/run.mjs --all\n    CWD> from tools/x/",
      ),
  ...
  let measured = 0;
  for (const [name, text] of declaredLimits) {
    if (text === good) {
      failures += 1;
      out.push(`  FAIL    a declared limit's degradation did not land: ${name}`);
      continue;
    }
    const r = run(rowsOf(text), corpus, name);
    if (r.findings === 0) {
      measured += 1;
      out.push(`  LIMIT   ${name} — NOT caught (0 findings), as declared: the cwd phrase resolves against the corpus, unpaired with its command`);
    } else {
      failures += 1;
      out.push(`  FAIL    a DECLARED LIMIT was caught (${r.findings} finding(s)) — the declaration in references/host-commands.md is stale: ${name}`);
    }
  }
  out.push(
    `degradations: ${cases.length} · caught ${caught} · missed ${cases.length - caught} · declared limits ${declaredLimits.length} · measured ${measured}`,
  );
