//! `supertaskr-index index --check` — is the committed graph current?
//!
...
pub fn render(report: &CheckReport, root_label: &str) -> String {
    ...
    let Some(stale) = &report.stale else {
        out.push_str(&format!(
            "[supertaskr-index] graph.json is CURRENT - {} matches a fresh index \
    ...
    out.push_str("[supertaskr-index] graph.json is STALE - the committed graph does not match a fresh index of this tree\n");
    out.push_str(&headroom_alarm(report));
    match stale {
        Staleness::Missing => {
            out.push_str(&format!(
                "[supertaskr-index]   committed:   MISSING at {}\n",
                rel_display(&report.graph_path, root_label)
            ));
        }
        Staleness::Unreadable(err) => {
            out.push_str(&format!(
                "[supertaskr-index]   committed:   {} bytes, but NOT a readable schema-1 graph\n\
                 [supertaskr-index]   | {err}\n",
                report.committed_bytes
            ));
        }
        Staleness::BytesOnly => {
            out.push_str(&format!(
                "[supertaskr-index]   committed:   {} bytes\n\
                 [supertaskr-index]   NO structural difference - the payload is identical and only the BYTES differ.\n\
                 [supertaskr-index]   That means a hand edit, a reformat, or a serializer change: ADR-014 says\n\
                 [supertaskr-index]   graph.json is never hand-edited, so regenerating is the whole fix.\n",
                report.committed_bytes
            ));
        }
        Staleness::Structural(delta) => {
            if let Some((cf, cs, ce)) = report.committed_stats {
                out.push_str(&format!(
                    "[supertaskr-index]   committed:   {} bytes · {cf} files · {cs} symbols · {ce} edges\n",
                    report.committed_bytes
                ));
            }
            out.push_str(&format!(
                "[supertaskr-index]   fresh index: {} bytes · {ff} files · {fs} symbols · {fe} edges\n",
                report.fresh_bytes
            ));
    ...
    out.push_str(&format!(
        "[supertaskr-index]\n[supertaskr-index]   regenerate: supertaskr-index index --root {root_label}\n"
    ));
    out
}
...
fn budget_line(report: &CheckReport) -> String {
    ...
    if used > budget {
        format!(
            "[supertaskr-index]   budget:      {used} of {budget} bytes ({percent:.1}%) - OVER by {}: \
             symbol arrays are being dropped (stats.truncated_symbols / truncated_files say how many)\n",
            used - budget
        )
    } else {
        format!(
            "[supertaskr-index]   budget:      {used} of {budget} bytes ({percent:.1}%) - {} left\n",
            budget - used
        )
    }
}
...
fn floor_line(report: &CheckReport) -> String {
    ...
    if floor >= budget {
        return format!(
            "[supertaskr-index]   floor:       {floor} of {budget} bytes ({percent:.1}%) - OVER: \
    ...
    if files == 0 {
        return format!(
            "[supertaskr-index]   floor:       {floor} of {budget} bytes ({percent:.1}%) - \
...
fn headroom_alarm(report: &CheckReport) -> String {
    ...
    if used > budget {
        return format!(
            "[supertaskr-index]\n\
             {drop}\
             [supertaskr-index] !! GRAPH HEADROOM ALARM - there is none left: this index is {} bytes past the\n\
             [supertaskr-index] !! ceiling and the emitter is ALREADY dropping symbol arrays to fit\n\
             [supertaskr-index] !! (stats.truncated_symbols / truncated_files carry the count). Nothing is red\n\
             [supertaskr-index] !! because nothing failed - the graph is valid, and smaller than the tree it\n\
             [supertaskr-index] !! describes. THE VERDICT ABOVE IS UNAFFECTED: this is the room left, not the\n\
             [supertaskr-index] !! answer. The threshold and what measured it: check::WARN_HEADROOM_BYTES.\n\
             {spend}\
             [supertaskr-index]\n",
            used - budget
        );
    ...
    if left >= WARN_HEADROOM_BYTES {
        ...
        return format!("[supertaskr-index]\n{drop}{spend}[supertaskr-index]\n");
    }
    format!(
        "[supertaskr-index]\n\
         {drop}\
         [supertaskr-index] !! GRAPH HEADROOM ALARM - {left} bytes left, under the {WARN_HEADROOM_BYTES}-byte tripwire.\n\
         [supertaskr-index] !! That threshold is ONE ORDINARY MERGE's growth of this graph, measured over this\n\
         [supertaskr-index] !! repository's own history - so the NEXT code lane can be the one that crosses,\n\
         [supertaskr-index] !! and it will be somebody who did not cause it and whose fence cannot fix it.\n\
         [supertaskr-index] !! Crossing does not fail: the emitter drops symbol arrays to fit and sets\n\
         [supertaskr-index] !! stats.truncated_symbols, so the map quietly stops answering what is in a file.\n\
         [supertaskr-index] !! THE VERDICT ABOVE IS UNAFFECTED: this is the room left, not the answer, and\n\
         [supertaskr-index] !! moving either number to quiet it is a value call rather than a fix.\n\
         [supertaskr-index] !! The threshold and what measured it: check::WARN_HEADROOM_BYTES.\n\
         {spend}\
         [supertaskr-index]\n"
    )
}
...
fn drop_clause(report: &CheckReport) -> String {
    ...
    if report.fresh_truncated_files == 0 {
        return "[supertaskr-index] !! GRAPH TRUNCATED - a fresh index of this tree set stats.truncated_symbols and\n\
                [supertaskr-index] !! recorded NO file count, which is apply_budget's FLOOR: it had nothing left to\n\
                [supertaskr-index] !! empty and emitted the over-budget document anyway. Degradation is finished\n\
                [supertaskr-index] !! here, so the headroom sentence below is the lesser of the two states.\n"
            .to_string();
    }
    ...
    let mut out = format!(
        "[supertaskr-index] !! GRAPH TRUNCATED - a fresh index of this tree DROPPED the symbol arrays of\n\
         ...
         [supertaskr-index] !! describes and will answer \"no symbols\" for files that have them.\n\
         [supertaskr-index] !! THIS IS THE LOUD ONE. Low headroom warns about the next merge; this is the\n\
...
fn spend_clause(report: &CheckReport) -> String {
    ...
    if spend < 0 {
        return format!(
            "[supertaskr-index] !! AND THIS WORKING TREE GIVES {} BYTES BACK: a fresh index is {fresh} bytes\n\
             [supertaskr-index] !! against the committed graph's {committed}. A map that shrank is only good\n\
             [supertaskr-index] !! news if the tree shrank with it - read it against any GRAPH TRUNCATED line\n\
             [supertaskr-index] !! above before taking it for relief.\n",
            -spend
        );
    }
    format!(
        "[supertaskr-index] !! AND THIS WORKING TREE SPENDS {spend} OF IT: a fresh index is {fresh} bytes\n\
         [supertaskr-index] !! against the committed graph's {committed}. THAT number is yours - the room\n\
         [supertaskr-index] !! left above is the same for every lane and goes stale on the reader; this one\n\
         [supertaskr-index] !! moved because of your diff.\n"
    )
}

fn render_delta(out: &mut String, delta: &GraphDiff) {
    if !delta.meta.is_empty() {
        out.push_str("[supertaskr-index]\n[supertaskr-index]   header\n");
        for line in &delta.meta {
            out.push_str(&format!("[supertaskr-index]   | ~ {line}\n"));
        }
    }
    section(
        out,
...
fn section(
    ...
    let mut header = format!("[supertaskr-index]   {label}");
...
mod tests {
    ...
    fn a_missing_committed_graph_is_stale_and_says_so() {
        ...
        assert!(text.contains("regenerate: supertaskr-index index --root ."), "{text}");
    }
    ...
    /// VERIFIER PROBE (T-167-s13, phase 2): the record reaches the report
    /// on the two arms a REAL repository takes — a committed graph.json
    /// that MATCHES the fresh index (CURRENT) and one that DIFFERS
    /// (STALE). Every shipped body reaches only the `Missing` arm,
    /// because `TempTree` starts with no graph at all, and `Missing` is
    /// the one arm no indexed repository ever takes.
    #[test]
    fn probe_the_record_reaches_the_report_on_the_current_and_stale_arms() {
        let t = TempTree::new("check-drop-committed");
        let fat: String = (0..400)
            .map(|i| format!("export const s{i:03} = {i};\n"))
            .collect();
        t.write("src/fat.ts", &fat);
        t.write("src/legitimately-symbol-less.ts", "// nothing to export\n");
        for i in 0..2 {
            t.write(&format!("src/thin{i}.ts"), "export const alpha = 1;\n");
        }
        let roomy = check(&opts(t.root())).unwrap();
        ...
        // ARM 2 — CURRENT: commit the truncated document itself.
        let fresh = index(&tight).unwrap();
        t.write(GRAPH_REL_PATH, &stable_json(&fresh));
        let current = check(&tight).unwrap();
        assert!(
            current.stale.is_none(),
            "the fixture must really reach the CURRENT arm: {current:?}"
        );
        assert_eq!(
            current.fresh_truncated_paths,
            vec!["src/fat.ts".to_string()],
            "the record must survive the CURRENT arm: {current:?}"
        );
        assert_eq!(
            printed_dropped_paths(&render(&current, ".")),
            vec!["src/fat.ts".to_string()]
        );

        // ARM 3 — STALE: move a file so the fresh index differs.
        t.write(
            "src/thin0.ts",
            "export const alpha = 1;\nexport const beta = 2;\n",
        );
        let stale = check(&tight).unwrap();
        assert!(
            stale.stale.is_some(),
            "the fixture must really reach the STALE arm: {stale:?}"
        );
        assert!(
            stale.fresh_truncated_paths.contains(&"src/fat.ts".to_string()),
            "the record must survive the STALE arm: {stale:?}"
        );
        assert!(
            render(&stale, ".").contains("WHICH FILES"),
            "and must still be PRINTED there"
        );
    }
    ...
    fn a_fresh_index_smaller_than_the_committed_graph_reads_as_giving_back() {
        ...
        assert!(
            text.contains("only good\n[supertaskr-index] !! news if the tree shrank with it"),
