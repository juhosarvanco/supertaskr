//! The `supertaskr-index` binary (T-014) — a shim, on purpose.
//!
//! Everything it does lives in the library (`supertaskr_index::cli::run`), so
...
fn main() {
    ...
    let code = supertaskr_index::cli::run(&argv, &mut out, &mut err);
