...
//! Regenerate with: SUPERTASKR_UPDATE_GOLDEN=1 cargo test -p supertaskr-index
...
fn check_fixture(name: &str) {
    ...
    assert_eq!(
        actual.as_bytes(),
        expected.as_slice(),
        "byte mismatch against {name}/expected-graph.json — regenerate deliberately with SUPERTASKR_UPDATE_GOLDEN=1 cargo test -p supertaskr-index"
    );
}
...
/// `SUPERTASKR_UPDATE_GOLDEN=1` moves both sides at once).
#[test]
