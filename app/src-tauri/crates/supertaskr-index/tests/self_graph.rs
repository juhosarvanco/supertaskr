...
//!
//!   cargo test -p supertaskr-index --test self_graph -- --ignored
//!
//! With SUPERTASKR_UPDATE_GOLDEN=1 that run REWRITES the committed graph —
//! the mechanism by which docs/architecture/graph.json is produced.

mod common;

use supertaskr_index::{index, stable_json, IndexOptions, GRAPH_REL_PATH};
...
#[test]
fn live_repo_walk_respects_the_dogfood_supertaskrignore() {
    // Content-independent shape checks: docs/ is the brain, not the
    // codebase (root .supertaskrignore), and dependency trees never walk.
    let graph = index(&self_options()).expect("index self");
    for file in &graph.files {
        assert!(
            !file.path.starts_with("docs/"),
            "docs/ must be excluded by the root .supertaskrignore: {}",
            file.path
        );
        assert!(
...
fn self_graph_is_current() {
    ...
    if common::update_golden() {
        let changed = supertaskr_index::write_graph(&graph, &committed).expect("write graph");
        eprintln!(
    ...
    assert_eq!(
        actual.as_bytes(),
        expected.as_slice(),
        "committed {} is stale — regenerate deliberately with SUPERTASKR_UPDATE_GOLDEN=1 cargo test -p supertaskr-index --test self_graph -- --ignored",
        GRAPH_REL_PATH
    );
}
