...
//! `CARGO_BIN_EXE_nputer-index` and reads the REAL exit status, because
...
fn check_is_green_then_red_on_a_planted_change_then_green_again() {
    ...
    assert!(
        red.stderr.contains("regenerate: nputer-index index --root"),
...
fn usage_mistakes_are_exit_two_from_the_real_process() {
    for args in [
        ...
        assert!(out.stderr.contains("[nputer-index] usage:"), "{args:?}");
    }
}

#[test]
fn a_gate_that_cannot_run_is_exit_three_never_exit_one() {
    let missing = "/nonexistent/nputer-t014-cli";
    for args in [
        ...
        assert!(out.stderr.contains("[nputer-index] FAILED:"), "{args:?}");
    }
...
fn help_and_version_document_the_exit_codes_the_process_actually_uses() {
    ...
    assert_eq!(version.stdout.trim(), format!("nputer-index {}", env!("CARGO_PKG_VERSION")));
}
