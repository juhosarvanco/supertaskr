...
mod tests {
    ...
    ///
    /// EXACT-SET semantics, both directions. A cycle that appears and is
    /// not listed here reds; an entry left here after its cycle is gone
    /// reds too. An allowlist that can only be too small is a rule that
    /// rots quietly, which is the failure this whole module exists for.
    ///
    /// 2026-08-25 (T-127) — `C-08 -> C-09 -> C-08` predates @human's
    /// no-cycles ruling by nine days (`T-033-s10`). Removing it is a
    /// re-partition of C-08/C-09's `paths:`, and that MOVES two live
    /// registry fixtures: measured on this lane at `afe23c1`, the
    /// smallest acyclic re-partition takes `npm test` from `app/` to
    /// **6 failed / 967 passed of 973, exit 1**, in
    /// `app/test/architecture-dogfood.test.ts` and
    /// `app/test/map-dogfood-render.test.tsx`. Both sit under
    /// `app/test/**`, which is C-05's `app-shell` — outside T-127's
    /// `[crate-index, docs/architecture/components/]` fence, so T-127
    /// could not reconcile them and routed the move as `T-127-s1`.
    ...
    fn the_live_registry_declares_exactly_the_cycles_this_crate_still_allows() {
        ...
        let components = registry::read_registry(&root).expect("the live registry must read");
        let report = cycles(&components);
        assert!(
            !report.truncated,
    ...
    fn the_live_registry_minus_one_hop_per_reported_cycle_is_acyclic() {
        // POSITIVE CONTROL against the LIVE corpus rather than a fixture:
        // the gate has to be able to answer GREEN about this tree, not
        // only RED, or "no cycles" would be a sentence it can never say.
        // Dropping the closing hop of every reported cycle in memory must
        // leave nothing behind — and on the day the last cycle is fixed
        // the removal set is empty and this body asserts the live tree
        // itself, so it does not go vacuous when the red goes away.
        let root = crate::testutil::repo_root();
        let mut components =
            registry::read_registry(&root).expect("the live registry must read");
        let before = cycles(&components);
    ...
    fn a_cycle_reintroduced_into_the_live_registry_is_caught_and_named_today() {
        // T-127's criterion: the gate SHALL make a reintroduced cycle
        // fail, and it SHALL fail TODAY. `C-06 -> C-01` is declared and
        // `C-01` declares nothing, so one added line closes a walk that
        // does not exist in this tree — the cheapest possible mistake a
        // future `depends_on:` edit could make.
        let root = crate::testutil::repo_root();
        let mut components =
            registry::read_registry(&root).expect("the live registry must read");
        let before = cycles(&components);
        ...
        assert!(
            c06.depends_on.iter().any(|d| d == "C-01"),
        ...
        assert!(
            render(&after, ".").contains("cycle  C-01 -> C-06 -> C-01\n"),
            "and printed as one"
        );
    }
}
