//! `nputer-index arch blast <path|slug>…` — how many things depend on
...
//!   `@nputer/parser` names the PACKAGE; which of that package's files
...
mod tests {
    ...
    fn the_package_seam_is_marked_rather_than_counted_as_zero() {
        ...
        let graph = graph_of(
            &files,
            vec![import("f:app/x.ts", "p:@nputer/parser")],
            vec![Package {
        ...
        assert_eq!(
            file.pkg_seam,
            Some(("p:@nputer/parser".to_string(), 1)),
            "a confident zero here is the failure mode the card is about"
        );
        assert!(render(&report, ".", "g.json", 1).contains("pkg-seam=p:@nputer/parser(importers=1)"));
    }
