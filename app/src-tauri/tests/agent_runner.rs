...
fn a_settled_turn_has_already_released_the_single_flight_latch() {
    ...
    match agent::send_turn(&h.watch, &h.agent, "answer".into()) {
        SendOutcome::Busy => panic!(
            "the terminal phase was published while the single-flight latch was still held:              a caller that polls status and then sends gets Busy for a turn that is over"
        ),
