...
export default {
  ...
  async degrade() {
    ...
    // ARM 4 — THE PARAGRAPH-BREAK FALSE POSITIVE. The separator between a
    // cited file and its ordinal must reach across ONE wrapped line and no
    // further: a line ENDING in a bare `<file>.md` and a LATER PARAGRAPH
    // OPENING with a token shaped `5d` are not a citation of each other.
    // Plant exactly that and require silence AND undiminished coverage.
    const across = degraded(clean, "method/lane-protocol.md", (t) =>
      t.replace("## The rules", `A pointer at ${CONTROL_HOME.replace("method/", "")}\n\n\n9q is nobody's sub-step.\n\n## The rules`),
    );
    const gapped = auditWithCoverage(across);
    arms.push(
      gapped.findings.length === 0 && gapped.lettered === base.lettered
        ? {
            ok: true,
            line:
              `a file name ending a paragraph and a \`9q\` opening the NEXT one are not ` +
              `read as a citation: 0 findings, and all ${gapped.lettered} lettered ` +
              `citations still examined`,
          }
        : {
            ok: false,
            line:
              gapped.findings.length > 0
                ? `a paragraph break between a file name and a lettered token was read as ` +
                  `a citation: ${gapped.findings[0]}`
                : `the paragraph-break plant dropped lettered coverage from ${base.lettered} ` +
                  `to ${gapped.lettered}`,
          },
    );
    ...
    return {
      ok: true,
      detail: `${arms.length} control arms hold: a baseline, 2 degradations, 2 false positives`,
      lines: arms.map((a) => a.line),
    };
  },
};
