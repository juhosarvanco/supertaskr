 ...
 * 2026-09-13: the third correction's `old` text matched
 * `.claude/hooks/hook-install.mjs` at TWO sites and its `new` text at
 * ONE — a presence check that legitimately preceded the executable check
 * the block was about. The reader this replaces answered that
...
function applyCorrectionStep(step, io) {
  ...
  // THE FILE AS IT WILL BE COMMITTED (T-295-s9): the merge is staged and
  // the working copy is what the commit takes, so the bytes on disk are
  // the subject of both counts.
