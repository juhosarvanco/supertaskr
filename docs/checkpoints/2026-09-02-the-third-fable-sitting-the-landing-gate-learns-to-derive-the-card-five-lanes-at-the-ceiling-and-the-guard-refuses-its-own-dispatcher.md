...
- HEALTH BANDS, over battery 19's own runner captures: at the pushed tree 763548c `health-bands: 14 band(s) — 5 inside, 2 drifting, 2 BREACHED, 1 unread, 4 UNKEPT` — the second breach was `docs-headroom/docs/STATE.md` (STATE at 8,421 bytes against an 8,465 warn line after the room-17/18 hazards landed, ROADMAP drifting at 12,131 against 12,252); content moved to this record and STATE was trimmed in the checkpoint commit (7,903 bytes); the ROADMAP trim failed twice on a wrapped-text anchor and landed in the follow-up commit right after, after which the same readings give `health-bands: 14 band(s) — 6 inside, 2 drifting, 1 BREACHED, 1 unread, 4 UNKEPT` — the remaining drifts are the two documents' headroom and triage/net-arrivals (this sitting filed cards faster than the window's line); a chain under `set -e` did not stop on a failing heredoc, which is why the checkpoint commit's message overstates the trim; and the follow-up commit that corrected this record without re-touching STATE made the docs gate read STATE as STALE for every path (docs-protocol rule 4 compares the two files' newest commits), so battery 20 reddened five bodies and the pair was re-committed together `suite/e2e-seconds` is still the breach (the e2e leg read 597 passed (9.3m), 597 bodies after T-238's twenty); T-120-s2 at priority 2 is the remedy and runs alone on tools/e2e in the first quiet window. Designed non-zero; never read as clean.
