...
Updated: 2026-09-14 at the unattended-night checkpoint by the architect
seat — the newest file in docs/checkpoints/ is **that record** (five
merges; method 0.1.27; the approval cards filed; batch 3a).
...
GREEN on the runner at 0d194f7c; read every run after it.**
...
- **THE MERGE IS THE ARM** (T-295, ADR-024 decision 3): `brief.mjs
  ...
  land in docs/checkpoints/meters.jsonl (T-297 parses them). **BUILD
...
- **THE BENCH IS TWO SPAWNS** (5d): phase 1 pasted the card at base +
  ...
  **POSTSCRIPT GROUND RULES: the heading shape; paths from the repo root.**
...
2. **LANES RUN IN PARALLEL WHERE FENCES ARE DISJOINT** (ruled
   2026-09-14; the order is in the record; only a record to show or a
   design decision parks). T-301 dormant: a screen is DESIGNED WITH THE
   OWNER before any build.
3. **WITH THE OWNER**: pile 2 batch 3b; T-242's delivery-and-fence
   draft; the T-301 design sitting; T-318's design; T-266 wakes
   2026-10-01.
...
- **A SUBAGENT THAT "FINISHES" WHILE ITS OWN JOB RUNS RE-FIRES**: never
  ...
  then continued fresh. **A `<-` IN CARD PROSE IS A PROVENANCE ARROW** to the
...
- **A RELAYED FACT IS A CLAIM**: say whose; never relay the attack set.
  **A FENCE OVERLAPPING A LIVE LANE PREFLIGHTS AT EXIT 1** (re-preflight
  after its merge).
...
- **BENCH RUNS USE PORT 25<card>**; **AN AMENDMENT UNDER ITS OWN HEADING
  ...
  WORD IN CARD PROSE AS UNSTAMPED** (preflight them); **SUBAGENT
  EFFORT IS THE APP SESSION'S**.
...
- docs/checkpoints/ — append-only; today's record is the newest;
  meters.jsonl is the bands' readings.
