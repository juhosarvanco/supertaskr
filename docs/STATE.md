...
Updated: 2026-09-13 at the rulings-day checkpoint by the architect
seat — the newest file in docs/checkpoints/ is **that record** (three
merges; method 0.1.26; the piles; the sitting).
...
GREEN on the runner at b7df8de2; read every run after it.**
...
never "fix" it; triage/live-suggestions BREACHES (T-306). **`--merge` MAY STOP FALSELY (T-295-s7/s8): rule through, finish by hand.** **AN EXIT MAY MEAN THE GATE NEVER RAN**:
...
2. **NO LANE IS LIVE** (T-301 dormant: a screen is DESIGNED WITH THE
   OWNER before any build). ONE LANE AT A TIME under the 2026-09-12
   authorization: T-314, the checkpoint-anchor fold, the T-312 rerun,
   T-313, T-315, T-316.
3. **WITH THE OWNER**: the `declarative` ruling (T-299-s6); pile 2's
   next batch; T-318's design; T-266 wakes 2026-10-01.
