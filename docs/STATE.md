...
never "fix" it; triage/live-suggestions BREACHES (T-306 wakes on
T-296, the owner's word). **`--merge` STOPS AT THE FORBIDDEN-SPELLING KEEPER ON PLANTED
...
- **THE MERGE IS THE ARM** (T-295, ADR-024 decision 3): `brief.mjs
  ...
  land in docs/checkpoints/meters.jsonl (T-297 parses them).
- **THE BENCH IS TWO SPAWNS** (5d): phase 1 pasted the card at base +
  ...
  **PASS `model` ON EVERY SPAWN and stamp what ran.** **A BENCH STANDS
