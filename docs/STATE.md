...
Updated: 2026-09-17 at the grant-leaves-the-template checkpoint by the
architect seat — the newest file in docs/checkpoints/ is **that record**
(four merges; method 0.1.35; the grant is an untracked store).
...
GREEN on the runner at be9726f1; read every run after it.**
...
2. **NO LANE IS LIVE.** T-344, T-332, T-335, T-295-s8 and T-205-s5
   merged 2026-09-17. **T-312, T-244-s1 and T-204 are FREE**; T-312
   opens T-313 and T-315, and T-315 opens T-316. An earlier note here
   said T-311 and T-314 blocked five cards — WRONG, both are `done`;
   **resolve a card by its `id:` field, never a filename glob, because
   `T-311-[a-z]` matches `T-311-s1`.** T-312 has a 2026-09-13 lane that
   reached `verifying` and never landed (`evidence/T-312-*`). Lanes in
   parallel where fences are disjoint; T-301 dormant (a screen is
   DESIGNED WITH THE OWNER before any build).
...
- docs/checkpoints/ — append-only; the newest is the 2026-09-17
  grant-leaves-the-template record; meters.jsonl is the bands' readings,
  and its nine newest entries are the INTEGRATOR's assembly, not seat
  blocks — they say so, and they do not cover every merged card.
