...
Updated: 2026-09-12 at the Codex-day-and-restart checkpoint by the
architect seat — the newest file in docs/checkpoints/ is **that record**
(T-300, T-303-s1 merged; ADR-025; nine plan cards; T-311 live).
...
GREEN on the runner at f7987bc8**; read the next run.
...
never "fix" it; triage/live-suggestions BREACHES (T-306's first batch is due). **`--merge` MAY STOP FALSELY (T-295-s7/s8): rule through, finish by hand.** **AN EXIT MAY MEAN THE GATE NEVER RAN**:
...
- **THE BENCH IS TWO SPAWNS** (5d): phase 1 pasted the card at base +
  ...
  **A BENCH STANDS
  UNTIL THE VERIFIER'S NOTIFICATION, never its verdict file.** **THE ARM
  RENDERS BOTH PHASES (T-296), NO VERIFIER PACK (T-296-s10), NO
  MEASUREMENTS ON THE STANDARD GROUND (T-296-s11)**: append the seat's
  addendum under the ground's heading, RESEAL by hand.
...
2. **T-311 IS THE LIVE LANE** (the run record; guarded); then, under
   the owner's standing authorization of 2026-09-12, ONE LANE AT A TIME
   in ADR-025's order (the record lists it); T-301 resumes after T-317,
   T-300-s6 and T-299-s6 land.
3. **WITH THE OWNER**: the pruning batch class by class (T-306), the
   rule-review sitting, the model-and-effort-settings room; T-266
   wakes 2026-10-01.
...
- **A SUBAGENT THAT "FINISHES" WHILE ITS OWN JOB RUNS RE-FIRES**: never
  ...
  **a criterion names only a path the fence carries**; **a filed card
  carries its notes and Verdicts sections from birth**.
