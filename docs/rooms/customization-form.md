...
## @architect (claude-fable-5-1, the Claude seat) — 2026-09-13, the settings portion of the reopened question

The vision this room serves, restated by the owner on 2026-08-31 and again on 2026-09-13: a person customizes how Supertaskr runs their project without opening a file. The mechanism under that is now settled and is not the open question: the files are the store, the terminal command is the one writer, and every surface renders the same reader (T-299, T-300, T-317, and the owner's mirror-first ruling of 2026-09-12 for the app). The owner recorded on 2026-09-13 that this answers the SETTINGS portion of the question reopened on 2026-08-31, and that what remains is the design of what a person does on each surface.

Three surfaces, each at a different point. The terminal lists the profile and every switch and carries the write flag; it has no guided flow for choosing a profile or changing a switch, and no preview of what a change resolves to. The app's settings screen (T-301) is a mirror by ruling; the edit is a follow-up card not yet cut, and the edit flow, the preview, the shape of a refusal on a forbidden combination, and the models section from T-318 on the same screen are undesigned. In Claude Code and Codex there is no separate interface and none is planned: a person asks the seat in chat and the seat runs the command; the seat skill does not yet say so, and it owes the same preview so the seat can explain before it writes.

Four things stay open beyond those surfaces, in the order the seat proposes to take them. First, the inventory of what is customizable at all: today only the typed settings are — the process switches and, after T-318, the models section — while the rules, skill-pack imports, attack-set sources and read-first lists are not typed and no surface can render them; that inventory decides how far the vision reaches, and it is the slot count this room already called load-bearing. Second, the pen for the rest: for typed settings the surface now writes the files, which is the symmetric form this room priced as c1; for rules and skills the earlier ruling stands, hand-authored files and an app that explains, and that is the tension this room recorded and has not resolved. Third, first setup: the profile is chosen at genesis, and nothing designs that first run, which is where this room's own counter-argument about a setup screen bites. Fourth, explanation: every surface owes one preview saying what a value resolved to and why it won, which T-318 carries for models and the process switches need as well. The seat's proposal: settle the inventory, design the one edit-and-preview flow once, render it three times. Nothing here reopens Q2 to Q9; Q2's propose-only pen moves with the second item when it is taken up.
