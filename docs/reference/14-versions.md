...
### method/ is the generic, product-agnostic convention
  ...
  v0.1.36 (T-205-s5, 2026-09-17) — the VERIFIER BRIEF release: method/roles/verifier.md gains the two sections the brief assembler reads for its role-specific rows — a `## The report` section, which row 12 names as its source, and one exit-write sentence in step 5, which row 11's stamp marker finds — both PURE INSERTIONS that displace no pinned sentence and leave step 5a and every reading restriction untouched; the role file deliberately does NOT gain a contract table of its own, because the thirteen-row table is ruled single ("the ONE place it lives") and a second row set is the defect the assembler now refuses by name, so a verifier's brief follows the executor's table with rows 4, 11 and 12 read against this file, the row numbers parsed out of the contract's own sentence rather than typed, the stamp step found by its marker rather than by a number, and the two-spawn frame fired by the role file's own sentence rather than by the role's name.
