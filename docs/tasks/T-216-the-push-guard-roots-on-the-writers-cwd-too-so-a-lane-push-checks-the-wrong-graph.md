...
## VERDICT — APPROVED at `c50bbf9`, 2026-09-01

Blind verifier, bench detached at the lane's BASE `9eb3ec8`. Verdict
`00f776ed…`; **seven phase-1 artefacts, every one stamped BEFORE the tip
existed** and all re-verified byte-identical at the verdict.

**THE BLINDNESS WAS A FACT ABOUT THE CLOCK, NOT A DISCIPLINE KEPT.** This
is the first card dispatched under `orchestrator.md` 5c — bench cut
alongside the lane rather than after it. There was no diff to decline to
read, and the verifier says so in its own verdict because a later reader
cannot tell the two apart.

### The finding: a claim the lane made in its own favour, disproved

The card said the misjudged verdict *"was never about the pushed tree, so
retiring it loses no measurement."* **The universal form is false.**

Measured on one fixture, graph STALE and token FRESH, base against tip —
**and the LANE re-derived it at 7 rather than transcribing the
verifier's 5**, dropping `cd -` deliberately because it lands wherever
`OLDPWD` points, which the line never names:

    cd <own> ; git push                          REFUSED graph-stale -> non-verdict
    pushd <own> && git push                      REFUSED graph-stale -> non-verdict
    cd "$OWN" && git push                        REFUSED graph-stale -> non-verdict
    git --git-dir=<own>/.git … push              REFUSED graph-stale -> non-verdict
    GIT_DIR=… GIT_WORK_TREE=… git push           REFUSED graph-stale -> non-verdict
    echo cd /elsewhere && git push               REFUSED graph-stale -> non-verdict
    cd ~ && cd <own> && git push                 REFUSED graph-stale -> non-verdict
                                                 0 of 7  ->  7 of 7

**Cross-checkout the claim holds exactly; same-checkout-unresolvable
retires a CORRECT verdict.** Not a defect in the code — fail-open on the
guard's own inability is this file's doctrine, criterion 3 asks for an
observable decline, and the remedy is printed at the refusal. A real cost
taken deliberately, because the alternative is guessing that an
unreadable line did not move the cwd, **and nothing distinguishes
"unreadable and it stayed" from "unreadable and it left." That is what
unreadability means.**

### Verified rather than read

The verifier built the mutants itself: M1=7, M2=2, M3=4, M4=3, identical
to the lane's, **containment checked pairwise rather than asserted.** The
`;`/`||` body the lane ADDED after catching M2 ⊆ M4 in its own drill is
exactly what keeps M2 independent.

Its predicted killer does not occur: reading a `cd` target and handing it
to `findCheckoutRoot` — which climbs silently to the writer's root — is
declined in every form tried (`~`, `"$LANE"`, `$LANE`, `;`, `||`,
`pushd`, `cd -`, bare `cd`, `--git-dir`, `GIT_DIR=`). Last-`cd`-wins is
correct.

**And the unmoved-cwd inference rests on a MEASUREMENT that contradicts
the documentation**: a shell `cd` does not persist across tool calls
here. The docs say it does. The guard depends on the measured behaviour.

### A positive control that failed its first check — inside the verification

Swapping in the base file produced exit 1 running **zero bodies** — an
import failure proving nothing. Redone as an API-preserving mutant, the
control dies properly. **The degenerate-control class, caught by a
verifier inside its own method.**

### The two e2e reds are not this lane's, pinned from BOTH directions

The lane showed `lane-lock:465` and `token-scan:273` persist at base in
an ARMED worktree. The verifier ran an UNARMED checkout of the tip:
`lane-lock` passes in full — **including the body carrying the cited
line, which is `T-210`'s own positive control** — and `token-scan` is 10
passed, exit 0. Two directions pin the arming as the variable. Routed as
a finding about `T-210`, not a red here.

### Recusal, declared in advance and honoured

Criterion 4's split clause and criterion 5 are the VERIFIER's text,
adopted onto the card by the dispatching seat. It pre-committed to
referring criterion 4 to the integrator if it were about to FAIL on it,
and to judging criterion 5 itself as binary. **It found the split clean
and said so plainly.** Authorship disclosed rather than laundered.

### Scope limit, stated rather than implied

It ran `push-guard.spec.ts` 56/56 and the two named reds. It did NOT
re-run the full 503-body lane — the browser specs are outside what its
scratch tree can host — so that figure rests on the lane's report.

### Non-blocking, routed

`cargo` now runs in a directory named by command text, bounded by the
crate-manifest check made BEFORE the spawn — folded into the header as a
widened threat model, explicitly not an escalation. And `T-216-s3`: a
push inside a subshell is not seen at all, where **the whitespace is the
entire difference** —

    ( cd /tmp && git push )   SEEN, unresolved (safe)
    (cd /tmp && git push)     NOT SEEN AT ALL

which makes it a MISLEADING DECLARATION rather than an honest gap.
