...
Absorbs: T-052-s5, T-091-s4, T-104-s4, T-124-s1, T-126-s6, T-132-s4, T-132-s5, T-133-s3, T-135-s4, T-145-s2, T-152 (and, transitively, the three ids these had themselves absorbed at the eleventh triage and the amnesty — T-138-s4 into T-152, T-132-s3 into T-133-s3, T-120-s1 into T-132-s4) — the ELEVEN parked riders this release TOOK, stamped 2026-08-30 by `T-159-s1`, the card triage promoted to write the frontmatter half this lane had no standing to write. Files removed in this commit; this line and the per-rider table below are the surviving record. Each was re-derived at `51fa31c` rather than read off that table, and each landing is a live sentence at that ref: **T-052-s5** → `integrator.md:5,12`, the top-level list is STEPS and a list inside a section is RULES. **T-091-s4** → `integrator.md:55` predicts the tree with `merge-tree --write-tree` and `:59` gives a mismatch the loudness a skipped gate gets; `git grep -n merge-tree -- method/` returned ZERO rows when the card was filed and returns that line now. **T-104-s4** → item 2 at `TASK-FORMAT.md:315-322`, which keeps the vocabulary copy and says why (the method must be readable before a project's code exists) and names MF-05 as the checker that makes redundancy safe — the POINTER arm the card preferred is refuted, not overlooked; item 4 was discharged at `a8d6df6` (T-093, 2026-08-27) which landed the miss-is-not-a-refutation rule at `docs/CONVENTIONS.md:462`; **items 1 and 3 carry no work and that is a RULING, not an oversight** — an ordinal cited for two different criteria, and *"each of the five"* where there are nine, both STAY AS RECORDED on T-104's done card, because correcting a criterion after its verdict changes what an adversarial verifier approved with no verifier watching. **T-124-s1** → arm A at `planner.md:25-41`, both spellings (bare git in the cwd, the runtime's write tool over a shell redirect), generalised off the one CLI exactly as the card's own caution demanded; arm B — the plan-interview banking-map row — is NOT here and is carried by `T-159-s4`. **T-126-s6** → `executor.md:82-96`, a gate derivation is not a figure, with the two-branch escape and the forecast preferred. **T-132-s4** → `lane-protocol.md:141`, the atomicity the collision test rests on is an obligation on the writer, so a permitted write is stage-and-commit in one motion. **T-132-s5** → `integrator.md:112`, ask WHO may write the fix before WHEN it became false. **T-133-s3** → the split its own first question asked for: the RULE at `orchestrator.md:55-60` (assemble with the project's own assembler, named in the project's conventions rather than here) and the SPELLING at `docs/CONVENTIONS.md:1098`. **T-135-s4** → repair 3 at `TASK-FORMAT.md:389-400`, a card dispatched in halves stays `building` and the finished half writes a dated line; repairs 1 and 2 refused for the card's own reasons. **T-145-s2** → the wording half at `docs/CONVENTIONS.md:344-363`, two tests either one sufficient (SHIPPED BYTES or GRAMMAR) with the adapter case ruled explicitly; its INSTANCE was already moot at v0.1.7 and its recorded cost of the T-145 narrowing survives on this line. **T-152** → arm 1 at `executor.md:15-18`, TASK-FORMAT's ceremony table as step 1's one addition with the reason clause intact; arm 2 was already built (`brief.mjs` row 11 emitted both size-S ceremony rows for T-159-s1's own dispatch, sourced from TASK-FORMAT's table, and said in as many words that WHICH row applies is a reading of the diff it will not guess); arm 3 refused as the card asked and the *"never work on main"* safety line was NOT deleted.
