...
## VERDICT — APPROVED at `34b9f61`, after one REJECTION, 2026-09-01

Blind verifier, bench detached at the lane's BASE `9d56b47`. Attack set
`675ec701173f7ac7a607d961ca4b961dbc32fac53be8a1e036682481ae82e01b`,
stamped before the diff was opened and **unchanged across both passes** —
the same standard judged the rebuild.

### Rejected first, on three normative sentences

Each stated a mechanism the repository does not have. None touched the
substance of either fast path.

1. **A positive control claimed for a state it was never measured in.**
   The law said the card's own file stayed writable *throughout* a
   half-performed widening. Driven directly against `decide()`, in that
   state **everything blocks** — the newly granted path, an already-held
   path, the card itself, and a fresh suggestion file. And both passages
   prescribe ROUTING as the remedy, which is a write under the refused
   directory: **the law told the reader to perform a remedy that is
   unperformable in exactly the window it describes.**
2. **The tripwire's carve-out was one class short.** Fast path B's
   dry-run targets a CHECKPOINT COMMIT, whose diff is the integration
   seat's standing writes — `docs/STATE.md` in **10 of the last 10**,
   `docs/checkpoints/` 10/10. Those are paths a card MAY fence, so the
   conflict lands inside an enforced write-set and the old text called it
   the evidence — the harm the same section names two paragraphs earlier.
3. **A reason generalised past its moment.** *"The fence a lane is judged
   by is the expanded manifest"* is true at the WRITE and false at the
   LANDING, which expands fresh from the card as committed.

### Repaired, and the repair went past what was asked

The rebuild states the measured shape, and adds the second-order half
nobody had written down: **routing cannot be written while the window is
open**, so the law now names a move that needs no write — report the
half-delivered widening to the seat that owes the other half — and says
the refusal is expected rather than a broken guard. The verifier then
extended the control past the lane's own drill:

    S1 (window OPEN)    a fresh suggestion file BLOCKS  (stale-stamp)
    S2 (window CLOSED)  that same file ALLOWS (always-writable),
                        while a never-granted path still BLOCKS

Gates at the tip: parser 344/344 · app 1131/1131 · cargo 627/0 · e2e
443/443 on a port derived from the card id · method-eval 0 with 6 evals ·
docs gate a verdict naming four suites. Every figure matched the lane's.

### A VERIFIER'S OWN FIGURE FALSIFIED BY THE LANE IT REJECTED

The verdict claimed **nine** cards fence `docs/STATE.md` or
`docs/checkpoints`. The lane derived **2**, two ways. A third structural
scan over 429 live cards agrees: `T-156-s1` and `T-157`. The counts
reconcile — 470 = 429 top-level + 41 under `docs/tasks/rejected/`.

Two mechanisms, both from asking a LEXICAL question where the claim
needed a STRUCTURAL one: `STATE` matched the English word in prose, and
line-scoped co-occurrence counted five cards **quoting** `T-157`'s fence
as declarations of it.

**Why it inflated toward its own finding, which is the transferable
half.** The cards that quote a fence line and write "STATE" in the same
breath are the cards ABOUT fence machinery — the same subject as the
finding — so the error term was **correlated with the hypothesis rather
than random**. And in the verifier's own words:

> I applied two standards of evidence inside one finding: I derived the
> 10/10 by listing actual commit diffs, but took the 9 from a one-line
> grep without spot-checking a single hit. **The weaker instrument went
> to the half that supported me.**

`card-preflight.mjs`'s header already records that a path-token scan over
card prose *"names path-shaped tokens in the thousands and misses in the
hundreds"* — read in phase 1, and the defect recreated anyway.

The finding survived: it rests on the 10/10, which three seats measured.

### A THREE-WAY DISAGREEMENT THAT WAS NOBODY'S ERROR

Three seats produced three forecast tree oids from one commit pair, all
agreeing on substance. `git merge-tree --write-tree` bakes the ARGUMENT
SPELLING into its conflict markers. Reproduced a fourth time at the
integration checkout:

    ... 53fe498      task/T-211-lane  ->  845c281e…
    ... 53fe498a0c62 task/T-211-lane  ->  c4d39e58…

**This vindicates the law's typing choice.** It types on whether a tree
was PRODUCED — spelling-invariant — where typing on the oid would have
made the rule unreproducible between seats. Carried as a corroboration on
`T-211-s1`, which owns that spelling.

### What this lane got right that its dispatcher did not

It was briefed with a false premise — that nothing gates a push — in
those words. **No sentence of the law mentions a push in either
direction.** It wrote instead: *"Name the ones your own project actually
holds when you adopt this section, and claim no others."* The verifier
checked specifically; the dispatcher's error never reached the law.

Its `decide()` finding is filed as **`T-228`**, with the counter-argument
tested to failure and the obvious fix shown to be wrong.
