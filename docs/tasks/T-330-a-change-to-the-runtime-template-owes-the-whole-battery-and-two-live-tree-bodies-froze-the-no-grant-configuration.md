...
which is mode-independent and keeps the derived list. It is carried on
T-333 as a candidate technique and not as the better remedy: it lets the
second copy proceed, but `copyFileSync` still carries the source's mode,
so the replacement lands read-only too and the fixture root is not yet
writable. T-333's own criteria hold the property either spelling has to
reach.
...
rejection arc changed. `docs/INDEX.md` is current and does not move. The
figure the census lands at is whatever the FINAL tree generates; a
candidate taken at an earlier tip is a cross-check, never a target to
preserve by hand, and a difference from it is explained rather than
edited away.

FIVE CORRECTIONS ASKED FOR IN REVIEW ON 2026-09-16, recorded here in the
seat's own section. The verdict below is a record and none of it is
rewritten; where a correction bears on a verdict cell the cell is named.

**The six-string grep is supporting inspection and does not carry the
exclusion.** Criterion 5's verdict cell reports zero hits for
`RUNTIME_TEMPLATE`, `dispatchBlock`, `grantState`, `method/runtime`,
`processSection` and `loadProcess` across all twenty excluded specs, and
reads that as "the drop exposes no configuration coverage at all". A grep
cannot establish that, because a spec may reach the configuration
transitively or by a name assembled at run time. WHAT CARRIES THE
EXCLUSION IS THE CONSUMER DERIVATION AND ITS BEHAVIOURAL CONTROLS: the
body in `gate-run.spec.ts` that derives every spec reading this project's
dispatch block and requires the narrowed selection to carry all of them,
and the four planted configuration defects in `cli.spec.ts` shown refused
...
records-only push that follows this merge. THE EXACT MEASUREMENT is two
figures from that push's own run: the interval from the push to the run's
creation, and the interval from creation to the last required job's
completion, each read from the run record rather than estimated, together
with the derived owed set that push actually selected. The obligation
survives a handoff and is carried in STATE until the trigger has occurred
and the evidence is recorded on this card.

**T-339 is FILED by this merge, not implemented by it.** The card names
two properties of the admission arm; filing a repair card is not the
repair. Until T-339 is built, the grant's compatibility with the
fixtures is what this card's verification established, and the grant's
AUTHORISATION INTEGRITY is a procedural check by the seat, not an
enforced guarantee. Nothing here should be read as "the grant is now
guarded".

**The grant landing in this merge was checked against the approved
artifact before it was committed.** The forty added lines of
`method/runtime/supertaskr.yaml` are byte-for-byte the forty added lines
of `dispatch-grant-revision-1-approved-2026-09-15.patch` in the handoff
bundle, whose recorded hash `1f3da850` recomputes; the diff removes
nothing. Scope preserved exactly: `revision: 1`, `at:
2026-09-15T07:22:18Z`, the same eight ids in `order` pinned to the same
eight blobs. This is a procedural check under the authorization already
given on 2026-09-15 and is not a second approval.

**Contention: no semantic result in these runs has been attributed to
it, which is weaker than the earlier wording.** The per-root solo lock in
`gate-run.mjs` serialises one root, not two worktrees on one host, and
the same source records a previous contention-induced Rust failure. So
the honest statement is that no particular pass or fail in these runs has
been attributed to contention, and that timings collected under shared
host load cannot support an isolated performance comparison. The earlier
claim that no pass or fail is affected is withdrawn. Criterion 5's
elapsed figures carry that qualification, and a quiet-host re-measure is
owed before any speed claim is made from them.
