...
- **SHAPE 2 — the attack set committed beside the verdict. REFUSED, and
  ...
  *"this card has to answer rather than assume"*. A lane executor
  ...
  `T-205-s6` (the architect seat ruled it so at b825e87), and the checker needs no change when it
...
- **SHAPE 3 — a gate at the landing.** The strongest, and outside this
  ...
  Routed as `T-205-s14`, parked behind `T-205-s6`.
...
outside this fence. Routed as correction 2 of the verdict below, at this merge.
...
spawn ran on `claude-fable-5-1` (its transcript and its verdict header say
so). The parser's assignment census refuses a `verified_by:` that differs
from `verifier:`, so both keys record the seat that verified; the executor
ran on `claude-opus-5` as stamped.
