...
## VERDICT — APPROVED at `e8bf4c1`, after one REJECTION, 2026-09-01

Blind verifier, bench detached at the lane's BASE `9d56b47`. Attack set
`2e5ed8a6bd20470f8f313efd10ef0afefd66ca7460ba7911f575e17ba6db8a23`,
stamped before the diff was opened and unchanged across both passes.

### Rejected first, on a positive control that had degenerated

The card requires the token be non-committable, and a body asserted it
with `git check-ignore`, using a tracked path as the control. **The body
passed for the wrong reason.** `.nputer/.gitignore` is written by the
FENCE writer at DISPATCH time, so non-committability was a property of
*having been dispatched as a lane* — and the integration checkout, where
pushes happen, is never armed as one. On a fresh clone:
...
condition the control existed to rule out. No mutant the lane could plant
would have reached it: the body is green in an armed worktree by
construction.

### Fixed, and pinned in the direction that matters

`writeToken` now arms the directory itself, with the ignore string in ONE
home imported by both writers. The lane **pinned the defect before fixing
it**: three new bodies run against the pre-fix writer, all three failing,
then the fix restored with an empty backup diff and re-run green.

The verifier drove this rather than reading it — landing its own mutant
at the site the property lives, confirmed one-for-one from `git diff`,
watching exactly those three bodies die, restoring byte-identical, 45/45.
It confirmed specifically that the bodies assert a **round trip onto
disk** and never `x === x`; the alias is nowhere compared to its source.

    precondition  nothing ignores .nputer/ yet        -> 1
    subject       check-ignore token                  -> 0
    control       check-ignore tracked path           -> 1   ← they DIFFER

### The three non-blocking findings, all graded FIXES rather than rewordings

**Cargo-less.** The refusal stands — an unrun suite genuinely is
unmeasured — but `REFUSED` no longer arrives as `token-red`. It is
`token-unmeasured`: the runner **declined to grade**. The verifier
supplied the test rather than its assent:

> A distinction is a rationalisation if it changes the label without
> changing what the system KNOWS or what the seat can DO.

This changes both — the graph arm has *no* information, the token arm has
a record stating rust was not graded. And the header opens by naming its
own contradiction: *"THIS FILE ARGUES THE OPPOSITE TWELVE LINES UP."*

**C-7.** Dirt is captured at WRITE time, so discarding it afterwards
cannot erase the record — the verifier graded that the right architecture
rather than a re-derivation that could be gamed. Modify tracked → mint →
discard → push is refused `token-unkeyed`.

**The card's false opening quote.** `docs/CONVENTIONS.md` never said what
this card's first line claimed. Zero occurrences at base and tip. The
RULE is real — earned at `18d8166` — and lives only in append-only
records, which is why a seat able to quote it broke it three times.
Routed: that bullet needs **writing**, not updating. Carried to `T-146`.

### THE LEDGER WAS A FIGURE WITHOUT A KEEPER, AND THAT WAS THE SECOND FINDING

"14 mutants, 0 survivors, M3 aborting mid-run" existed only in the lane's
report. **The verifier could name the gap and not close it** — there was
nothing in the tree to read — which is what made it a defect rather than
a nitpick.

Now stamped, and **generated from `drill-ledger.json` rather than
retyped**: every landing is what `diff` printed, every kill list the
runner's own body names, every restore a sha256 taken before mutation.
Counts reconcile one-for-one.

**Two deliberate non-edits, both the same rule.** The ledger records M11
killing a body renamed *after* that run, and the transcription was NOT
corrected: *a ledger is a record of a run at a ref, not a description of
the current tree; correcting it would make it tidier and false.* And no
fourth CAPABILITIES figure was added, because writing one creates a new
tip and changes it again — the card carries three ref-stamped figures and
an instruction to re-derive, which is the handling that terminates.

### WHAT THIS CARD COSTS, DISCLOSED RATHER THAN BURIED

**Every push now owes the full four-suite battery, run last.** A
cargo-less checkout cannot push at all, and every prose commit stales the
token — so a verifier appending a verdict owes the whole battery
including the solo e2e lane. That is the card's stated design, it is
announced at the point of refusal, and it is `push-guard.mjs`'s own
*"expensive enough that it gets turned off"* case arriving in practice.
**Reversible in one `git revert` of this merge** if the seat that ships
judges the cost wrong.
