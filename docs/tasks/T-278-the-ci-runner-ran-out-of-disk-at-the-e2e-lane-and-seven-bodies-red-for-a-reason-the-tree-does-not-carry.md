...
## Corrections at the merge (the architect seat, 2026-09-09, T-278's verdict)

- C3 — the Implementation notes' clone census says "exactly three `git clone` call sites" and lists four (brief.spec.ts:2296 and :2306, session-economics.spec.ts:202, checkout-currency.spec.ts:289); four is the figure a grep at 4327aff finds. The conclusion (every clone is of a one- or two-commit scratch repository) stands; the count was typed.
- C4 — criterion 2's "before any suite runs" was NOT built in letter, and this is the reading taken: the pair brackets the e2e lane (the floor at step index 24; the parser, app and cargo suites at 9, 14 and 15 precede it), because the job head would measure the wrong moment — the caches, the cargo build and the app build all land between the job's head and the lane, so a floor at the head reads a disk the lane never sees (the "moved reading" mutant's own message). The card's measured section asked for exactly the pair that landed.
- C1 and C2 performed in the tree at this merge: the two comments' "step 4 (fence)" attribution corrected (two of the four attempts stopped at step 6, the bench), and the live keeper gained the executing arm — the floor step's own shell run against a stub `df` one KiB above and one KiB below the floor — so its name, "behind a floor that can fire", is derived; drilled with the `&& false` landing, RED by the keeper alone.
