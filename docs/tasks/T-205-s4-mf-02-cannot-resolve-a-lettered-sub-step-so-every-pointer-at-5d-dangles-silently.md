...
## The review field, stamped at the merge (the integrator, 2026-09-09)

`review:` was EMPTY at dispatch (the verdict's correction 2): the field is
set at dispatch and the requirement never reached a seat that could honour
it. Stamped `independent` at the merge — the guarantee that actually held: the
executor and the phase-2 verifier both ran on claude-opus-5 as blind seats
in the two-spawn construction. The shortfall is recorded in the checkpoint.
