...
## The assigned correction, performed at the merge (the integrator, 2026-09-09)

The third verdict's correction, half 1, taken as written: `touchesAmendments`
collects an inherited ambiguity into a third list, `unjudged`, instead of
returning `{ problem }` from inside its loop; it returns
`{ moved, duplicated, unjudged }`; both verdict arms refuse on `moved` and
`duplicated` with the unjudged ids named beside the refusal
(`unjudgedReport`), and where nothing is refused the lane arm's allow names
them and the merge arm carries them in its `unjudged` list. The words of
limit 5(f) ("stops judging that id") are now what the code does; no prose
moved. The body owed is the last in `landing-gate.spec.ts` ("an ambiguity
the range INHERITED on one id does not discard the refusal found for
another — it is announced beside it"): five arms — the widening beside the
ambiguity, the arrived duplicate beside it, the control with the ambiguous
card untouched, and the announced allow kept. The drill, run through the
wired hook at this merge's tree: the body RED against the hook as verified
at 63b9c3d (the natural mutant: `an inherited ambiguity discarded the
widening refusal … landing-gate-cannot-compare`), GREEN after the patch
(1 passed); the whole spec's count is in the merge's checkpoint record.
