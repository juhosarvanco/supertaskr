...
- Every path the diff touches is one of the card's eleven `touches:` entries or a card under
  `docs/tasks/`. The fixture git identity in the new bench is the suite's own synthetic identity (the one spelling redacted here by the integrator on the keeper's instruction, 2026-09-14), which
