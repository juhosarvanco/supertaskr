...
## Note at the merge (the integrator, 2026-09-09, T-224's landing)

The letter this card asks for has moved: T-224's third pass (63b9c3d)
took `(f)` for the inherited-ambiguity condition ("an id already carried
twice when the range was cut is the honest cannot-compare"), so the entry
naming the direct non-merge commit on the integration branch is the NEXT
letter, `(g)`. Everything else above stands; the third verdict named this
card's route again as the one road left to an inherited ambiguity.
