...
## INTEGRATOR'S CORRECTION at the merge, 2026-09-02

The executor's sentence that only this fix mints a 64-hex lock name, so every 64-hex lock on the machine is this lane's, was true when written and false the moment the verifier's bench was cut at the tip: the bench carries the fix too. A present-tense claim about a moving machine, stated as a standing property (CONVENTIONS' class). The executor flagged it rather than editing under a verdict that cites 18c88e3; the correction lands here, beside the stamp, so the verdict's tip is untouched.
