...
### The seat's landing record — criterion 3

2026-09-15, the architect seat. The lane recorded no figures and invented none,
which was right: this criterion's trigger fires when the card LANDS, so nothing
at the lane tip could satisfy it. What follows is the definition the figures are
taken against, the before side, and what is still owed and when it arrives.

**THE TWO CLOCKS, WITH THEIR START AND STOP EVENTS NAMED.** Neither is a wall
time anybody may read a convenient way.

- **Time to push** runs from the moment the landing commit exists in the
  integration checkout to the moment `git push origin main` returns
  successfully. It therefore INCLUDES the owed-set run the push must pass and
  the pre-push guard's own judgement, because those are what a seat actually
  waits through before the remote has the work.
- **Time to CI completion** runs from the run's creation to the completion of
  the LAST required check — never the fastest job, never a sum of jobs, and
  never the run's own updated timestamp where that differs.

Every figure below carries the run identifier and the ref it was measured at. A
duration with no run identifier is not a measurement and is not recorded here.

**WHAT A RECORDS-ONLY PUSH IS, STATED HERE BECAUSE THE TREE DEFINES NOTHING.**
The ground taken at the base establishes that this repository has no
`paths-ignore`, no `paths:` filter and no mechanical notion of the category: the
phrase exists only in prose in the derivation's own comments and in the lane
conventions. For this card a records-only push is a push whose range changes
only paths under `docs/`. Nothing in the tree will supply that definition, so
using the figures against any other definition makes the comparison unanchored.

**THE BEFORE SIDE, FROM THE RUNNER ITSELF.** Run 34946192300 at
`2cad64971e0892efc393c34df6e3924e3937e893`, created 2026-09-15T08:17:59Z and
completed 08:55:22Z.

- time to CI completion: **2243 s (37 m 23 s)**. The critical path is the
  end-to-end shard carrying the docs-input-gate spec, at 2222 s (37 m 2 s); the
  planning job itself took 15 s and the checks job 90 s.
- time to push: **NOT MEASURED, and not reconstructed.** That push was the
  previous seat's and no clock was taken. The card records the absence rather
  than inventing a number to compare against.
- the selection that run actually used, from the planning job's own log:
  `suites=app,e2e,parser,rust`, `e2e-whole=true`, `spec-count=42`,
  `run-rust=true`, `run-boot=true`, preceded by
  `ci-owed: THE DERIVATION FAILED CLOSED`, whose sentence names both unresolved
  edges. That log line is the evidence the fallback was USED, rather than an
  inference from the run being green.

**WHY THIS CARD'S OWN LANDING PUSH IS EXPECTED TO SHOW NO NARROWING, AND WHY
THAT IS THE CRITERION WORKING RATHER THAN FAILING.** The derivation cannot place
`.github/workflows/ci.yml` — it lies under no package root, no spec reaches it
through a static import, and no docs mapping carries it — so any range touching
this card's own repair falls closed to the whole battery whatever the repair
does. The lane-cut keeper refused the scoped reading for this very fence and
said so. The narrowing is therefore visible on the NEXT push and not on this
one, which is exactly why the criterion asks for two pushes rather than one.

**WHAT REMAINS OWED, AND WHEN IT ARRIVES.** The landing push's two figures and
its selection are recorded by the seat as they are measured: the push clock at
the push, the CI clock when that run completes. The records-only push's figures
follow from the commit that records them, which is itself a change under
