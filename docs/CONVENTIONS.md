...
- **AND SINCE T-280 A PUSH OWES THE SET ITS OWN RANGE OWES, NOT THE
  ...
  **IT FAILS CLOSED, AND THAT IS THE WHOLE SAFETY ARGUMENT.** A path
  ...
  token and printed at the refusal. This mechanism can only ever be
  wrong by owing too MUCH. A docs path NOTHING reads is the one positive
...
- method/ is the generic, product-agnostic convention — nothing
  ...
  method/ formats are version-bumped (currently v0.1.14) and noted here.
