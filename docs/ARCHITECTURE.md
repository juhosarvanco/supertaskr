...
| C-05 | App | Front door (ADR-008): Tauri shell + panes over files; hosts the milestone-1 watcher (T-003); see docs/design/dashboard.md | C-01, C-06; C-07 when F-06 lands | building (board + map panes complete T-001…T-012; T-026 landed the genesis front door and the full-bleed `genesis` screen, but C-13's lens is still an unmounted slot inside it; rooms/sessions pending F-05) |
...
- Code layout: `app/` = C-05 (Tauri 2 + React + Vite + Tailwind/shadcn;
  ...
  T-026 mounted the shell's `genesis` SCREEN around it
  (`app/src/components/shell/GenesisScreen.tsx`, C-05) but the lens
  itself is still an unmounted slot — the two branches ran in parallel
  from a common base, so nothing on main imports `app/src/genesis/**`
  yet and Rollup still drops it) · `lib/parser/` = C-06,
