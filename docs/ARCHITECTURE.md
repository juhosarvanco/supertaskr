...
| C-05 | App | Front door (ADR-008): Tauri shell + panes over files; hosts the milestone-1 watcher (T-003); see docs/design/dashboard.md | C-01, C-06 | building (shell, watcher, board, detail panel, picker done — T-001/T-003/T-004/T-005/T-007; design pass T-006 pending) |
...
- Code layout: `app/` = C-05 (Tauri 2 + React + Vite + Tailwind/shadcn;
  ...
  package. Each package owns its package.json; app depends on
  @nputer/parser via file:../lib/parser (T-003; parser builds before
  app — ADR-011); no root workspace until a third package forces one.
