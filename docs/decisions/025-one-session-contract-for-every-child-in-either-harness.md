...
### Addendum — 2026-09-19: the Codex-only takeover uses a demonstrated hook profile

The owner chose a Codex hook-based writer profile for the current takeover after reviewing the qualification's successful edit, refusal, commit and browser checks and its limitations. For this profile, decision 5's requirement that the Codex sandbox confine writes to the lane's Git state is superseded: the safeguards are the configured tool hooks, file permissions where applicable, the child's path check and the authoritative landing check. They guard ordinary mistakes and do not enforce OS filesystem, read or Git-ref isolation. A hook may fail while the runtime proceeds with the operation; shell operations and deliberate bypass can escape these safeguards. An adapter must identify the profile and its effective hook registration, refuse unestablished eligibility, surface observed hook faults and reconcile affected work before continuing. Repository separation does not itself supply an access-control boundary.

T-312 must demonstrate the implemented route for the executor and fresh verifier separately, including lifecycle operations and exact candidate and correction collection. T-315 remains the separate complete Codex-only delivery acceptance. A native spawn does not inherit eligibility from the CLI demonstration. Live Claude and mixed-harness demonstrations are deferred; the shared run record, independent verification, seals, grant admission, push and landing checks remain. A separately qualified sandboxed mode is possible later, with its stronger claims kept distinct; building a separate runner is not a prerequisite for this takeover.

For phase one, the owner accepts a fresh session instructed to use only its frozen evidence packet, to invoke no tools or delegation, and to request missing evidence from the coordinator. The packet may include identified baseline source and measurements, but not the new executor candidate or reasoning. This is a procedural restriction, not mechanical removal of tools or filesystem read isolation. The fresh phase-two session and disclosure of shared old material remain required. Earlier records and verdicts are retained as history and are not regraded by this change. This addendum records the chosen policy; actual card text, grant pins and dispatch still use their existing instruments.


### Local coordination profile — 2026-09-23

For the revised T-312 local-coordinated profile, decision 5 and the hook-profile addendum no longer require per-turn hook registration receipts or qualification-only model sessions as a condition of delivery. The coordinating arm retains ownership of admission, workspace reservations, fence widening and acceptance. Supported hooks provide early refusals and automatic checks after operations; detected violations hold the affected attempt, preserve work and require confirmed process/job reconciliation. A post-operation result is not itself proof that execution stopped. Complete independent checks of the candidate and verified tip, fresh verification and the shared push/landing gates remain. Repository separation guards accidental interference and does not claim filesystem or Git-ref confinement. An isolated profile remains separately selectable and qualified. Earlier verdicts are not regraded. T-315 retains the actual Codex-only delivery acceptance; live cross-harness demonstrations remain deferred.
